...
/* helper function to subdivide a triangle and normalize the lengths from the origin */
void triangle_subdivide( Module *md, Polygon *p, int divisions );
...
void triangle_subdivide(Module *md, Polygon *p, int divisions) {
    if (divisions == 0) {
        ...
        Point m01, m12, m20;
        point_mid(&p->vertex[0], &p->vertex[1], &m01);
        vector_normalize(&m01);
        point_mid(&p->vertex[1], &p->vertex[2], &m12);
        vector_normalize(&m12);
        point_mid(&p->vertex[2], &p->vertex[0], &m20);
        vector_normalize(&m20);

        // Define vertex arrays for the subdivided triangles
        Point vp[4][3] = {
            {p->vertex[0], m01, m20},  // First subdivided triangle
            {m01, p->vertex[1], m12},  // Second subdivided triangle
            {m12, m20, m01},           // Third subdivided triangle
            {m20, m12, p->vertex[2]}   // Fourth subdivided triangle
        };

        for (int i = 0; i < 4; i++) {
            polygon_set(&subp[i], 3, vp[i]);  // Set the subdivided polygons
            triangle_subdivide(md, &subp[i], divisions - 1);  // Recursive call
        }
...
/**
 * Adds a unit sphere, centered at zero to the Module 
 * it just adds polygons that resemble a sphere to the module
 */
void module_sphere( Module *md, int divisions ) {
  ...
  point_set3D( &bottomLeft, -0.5, 0, -0.5 );
  vector_normalize( &bottomLeft );
  point_set3D( &bottomRight, 0.5, 0, -0.5 );
  vector_normalize( &bottomRight );
  point_set3D( &topRight, 0.5, 0, 0.5 );
  vector_normalize( &topRight );
  point_set3D( &topLeft, -0.5, 0, 0.5 );
  vector_normalize( &topLeft );

  point_set3D( &xtop, 0, 1.0, 0.0 );
  point_copy( &pt[0], &bottomLeft); //bottom left
  point_copy( &pt[1], &topLeft); // top left
  point_copy( &pt[2], &xtop); 
  polygon_set( &p[0], 3, pt);

  point_copy( &pt[0], &topLeft); // top left
  point_copy( &pt[1], &topRight ); // top right
  polygon_set( &p[1], 3, pt);

  point_copy( &pt[0], &topRight ); // top right
  point_copy( &pt[1], &bottomRight); //bottom right
  polygon_set( &p[2], 3, pt);

  point_copy( &pt[0], &bottomRight); //bottom right
  point_copy( &pt[1], &bottomLeft ); //bottom left
  polygon_set( &p[3], 3, pt);  

  point_set3D( &xtop, 0, -1.0, 0.0 );
  point_copy( &pt[2], &bottomLeft ); //bottom left
  point_copy( &pt[1], &topLeft ); // top left
  point_copy( &pt[0], &xtop);
  polygon_set( &p[4], 3, pt);

  point_copy( &pt[2], &topLeft); // top left
  point_copy( &pt[1], &topRight); // top right
  polygon_set( &p[5], 3, pt);

  // right side triangle
  point_copy( &pt[2], &topRight ); // top right
  point_copy( &pt[1], &bottomRight); //bottom right
  polygon_set( &p[6], 3, pt);

  // bottom side triangle
  point_copy( &pt[2], &bottomRight); //bottom right
  point_copy( &pt[1], &bottomLeft ); //bottom left
  polygon_set( &p[7], 3, pt);

  // at this point, p[8] is the octahedron that will make the sphere with unit radius 1
  for (int i = 0; i < 8; i++) {
    triangle_subdivide( md, &p[i], divisions );
  }
}
